...
onMounted(() => {
  loadVisitStatsData()
  if (areaRef.value) {
    areaRef.value.style.position = 'relative'
  }
})

const width = ref('50px')
const changeWidth = () => {
  width.value = width.value === '50px' ? '340px' : '50px'
}
/** 拖拽 */
...
const handleMouseMove = (e: MouseEvent) => {
  if (!isDragging || !areaRef.value) return

  const { left, top } = areaRef.value.getBoundingClientRect()
  const maxX = areaRef.value.clientWidth - parseInt(width.value) - 15 // 假设拖拽元素宽高为100px
  const maxY = areaRef.value.clientHeight - parseInt(width.value)

  itemX.value = Math.min(Math.max(e.clientX - offsetX - left, 0), maxX)
  itemY.value = Math.min(Math.max(e.clientY - offsetY - top, 0), maxY)
}
...
const onMouseDown = (e: MouseEvent) => {
  ...
  isDragging = true
  offsetX =
    e.clientX - (itemX.value + areaRef.value.getBoundingClientRect().left)
  offsetY =
    e.clientY - (itemY.value + areaRef.value.getBoundingClientRect().top)
...
<template>
  <div class="dashboard-container" ref="areaRef">
    <div
      class="clock-box"
      :style="{
        width,
      ...
      @dblclick="changeWidth"
      @mousedown="onMouseDown">
      <Clock :height="width" v-if="width === '340px'" />
      <div v-else class="wh-full flex-center">
        <el-icon size="30"><AlarmClock /></el-icon>
      </div>
    </div>
    ...
    <el-row :gutter="10" class="mt-5">
      ...
      <el-col :xs="24" :span="8">
        <el-card>
          ...
          <el-scrollbar height="400px" class="pr-15px">
            <div
